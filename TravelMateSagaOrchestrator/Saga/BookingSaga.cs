    ...
    public BookingSaga()
    {
        ...
        DuringAny(
            ...
            When(PaymentFailedEvent)
                ...
                .Finalize(),
            When(PaymentFinalizedEvent)
                .If(context => context.Saga.CurrentState.Equals(Final.Name), then => then.Then(
                    context =>
                    {
                        Console.WriteLine($"{context.Saga.CurrentState} state reached. Booking finalized - skipping incoming events.");
                    })
                .Finalize()
            )
        );
